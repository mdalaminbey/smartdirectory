<?php
/**
 * SmartDirectory routeing core system
 *
...
class Route {
	...
	protected static $group_configuration = array();
	...
	public static function group( $prefix_or_configuration, \Closure $routes ) {
		...
		$routes();
		self::$group_configuration = array();
	}
	...
	private static function register( string $path, $callback, $method ): void {
		...
		$args = array(
			...
			'callback'            => function ( WP_REST_Request $wp_rest_request ) use ( $callback ) {
				...
				$response = self::get_response( $callback, $wp_rest_request );
	...
	protected static function get_response( $callback, WP_REST_Request $wp_rest_request ) {
		...
		if ( is_array( $callback ) && count( $callback ) === 2 ) {
			 ...
			 * Get dependencies
			 */
			$dependencies = self::get_dependencies( $parameters, $wp_rest_request );
	...
	/**
	 * Format api regex for wp `rest_get_server()->register_route()`
	 *
	 * @param string $route
	 * @return array
	 */
