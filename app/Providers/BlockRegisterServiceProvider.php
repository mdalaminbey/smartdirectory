...
class BlockRegisterServiceProvider extends ServiceProvider
{
    ...
    public function action_init(): void
    {
        ...
        wp_register_script( 'smart-directory-block', Common::asset( 'js/block.js' ), $asset_file['dependencies'], $asset_file['version'] );
        wp_localize_script( 'smart-directory-block', 'Block', [
            'items' => json_encode( $block_list )
        ] );

        foreach ( $block_list as $key => $block ) {
            register_block_type( 'smart-directory/' . $key, [
                'api_version'     => 2,
